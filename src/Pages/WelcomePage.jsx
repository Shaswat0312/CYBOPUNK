...
<div className="absolute -inset-1 rounded-2xl border-4 border-cyan-400 blur-xl opacity-20 animate-pulse pointer-events-none"></div>
        ...
        <form onSubmit={handleStart} className="flex flex-col items-center gap-6">
          <input type="text" placeholder="TYPE YOUR ALIAS..." value={input} onChange={(e) => setInput(e.target.value)}className="w-full px-6 py-4 rounded-md bg-black/50 text-cyan-200 font-mono border border-cyan-500/30 focus:outline-none focus:ring-2 focus:ring-cyan-400 placeholder:text-cyan-400 placeholder:tracking-widest text-lg" />
      <button type="submit" className="w-full py-4 text-xl font-bold text-white bg-gradient-to-r from-cyan-600 to-blue-800 rounded-lg tracking-widest shadow-md hover:shadow-cyan-400/40 hover:scale-105 transition-all duration-300">
